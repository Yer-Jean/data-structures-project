...
class Queue:
    ...
    def dequeue(self):
        ...
        """
        if self.head is None:  # Если очередь пуста, то и нечего удалять - выходим из метода
            return

        # Удаляемый узел - это головной узел
        deleted_node = self.head

        # Теперь головным узлом становится следующий
        self.head = self.head.next_node

        # Если головного узла нет, значит очередь пуста - и хвоста тоже нет
        if self.head is None:
            self.tail = None

        # Возвращаем данные удаленного узла, бывшей головы очереди
